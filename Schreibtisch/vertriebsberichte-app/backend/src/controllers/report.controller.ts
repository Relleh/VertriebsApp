...
import { ReportInput, UpdateReportInput } from '../validators/schemas';

export const createReport = (
  req: Request<{}, {}, ReportInput['body']>,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const customerResult = query(
      'SELECT id FROM customers WHERE kunde_nr = ?',
      [kundeNr]
    );

    if (customerResult.rows.length === 0) {
      const userRegion = query(
        'SELECT region_id FROM users WHERE id = ?',
        [userId]
      );
      
      const newCustomer = query(
        'INSERT INTO customers (kunde_nr, name, region_id, klassifizierung, is_neukunde) VALUES (?, ?, ?, ?, ?) RETURNING id',
        [kundeNr, kundeName, userRegion.rows[0].region_id, klassifizierung, neukunde]
      );
      ...
      query(
        'UPDATE customers SET klassifizierung = ?, is_neukunde = ?, updated_at = datetime(\'now\') WHERE id = ?',
        [klassifizierung, neukunde, customerId]
      );
    }
    ...
    next(new AppError(`Fehler beim Erstellen des Berichts: ${errorMsg}`, 500));
  }
};
...
export const getReports = (
  ...
  try {
    ...
    const sortField = validSortFields[sortBy as keyof typeof validSortFields] || 'r.datum';
    ...
    const reports = query(queryText, queryParams);
    const countResult = query(countQueryText, countParams);
    ...
    next(new AppError('Fehler beim Laden der Berichte', 500));
  }
};
...
export const getReport = (
  ...
  try {
    ...
    if (report.rows.length === 0) {
      return next(new AppError('Bericht nicht gefunden', 404));
    }
...
export const updateReport = (
  req: Request<{ id: string }, {}, UpdateReportInput['body']>,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : 'Unbekannter Fehler';
    next(new AppError(`Fehler beim Aktualisieren des Berichts: ${errorMsg}`, 500));
  }
};
...
export const deleteReport = (
  ...
  try {
    ...
    const result = query(
      'DELETE FROM reports WHERE id = ? AND user_id = ? RETURNING id',
      [id, userId]
    );
...
export const getRegionalStatistics = (
  ...
  try {
    ...
    const statistics = query(queryText, queryParams);
...
export const getGlobalStatistics = (
  ...
  try {
    ...
    const globalStats = query(globalQueryText, queryParams.slice(0, queryParams.length / 3));
    const employeeStats = query(employeeQueryText, queryParams.slice(0, queryParams.length / 3));
    const topCustomers = query(topCustomersQueryText, queryParams.slice(0, queryParams.length / 3));
    const trends = query(trendsQueryText, []);
    ...
    next(new AppError('Fehler beim Laden der globalen Statistiken', 500));
  }
};