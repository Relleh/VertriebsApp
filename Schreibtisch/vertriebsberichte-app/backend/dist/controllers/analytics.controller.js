...
exports.getPerformanceComparison = exports.getTeamAnalytics = exports.getPersonalAnalytics = void 0;
...
const getPersonalAnalytics = async (req, res) => {
    try {
        ...
        if (!userId) {
            throw new error_middleware_1.AppError('Benutzer nicht authentifiziert', 401);
        }
    ...
    `;
        const personalStats = (0, database_1.query)(personalStatsQuery, [userId, dateFrom.toISOString().split('T')[0]]);
        // Neueste Aktivitäten
      ...
      LIMIT 10
    `;
        const recentActivity = (0, database_1.query)(recentActivityQuery, [userId, dateFrom.toISOString().split('T')[0]]);
      ...
      LIMIT 6
    `;
        const monthlyTrends = (0, database_1.query)(monthlyTrendsQuery, [userId]);
        // Status-Verteilung
      ...
      ORDER BY count DESC
    `;
        const statusDistribution = (0, database_1.query)(statusDistributionQuery, [userId, dateFrom.toISOString().split('T')[0], userId, dateFrom.toISOString().split('T')[0]]);
        // Formatiere monatliche Trends für bessere Lesbarkeit
        const formattedMonthlyTrends = monthlyTrends.rows.map((trend) => ({
...
exports.getPersonalAnalytics = getPersonalAnalytics;
/**
...
const getPerformanceComparison = async (req, res) => {
    ...
    `;
        const currentPeriod = (0, database_1.query)(currentPeriodQuery, [userId]);
        const previousPeriod = (0, database_1.query)(previousPeriodQuery, [userId]);
